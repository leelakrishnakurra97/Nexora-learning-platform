...
function RoomJoinFallback() {
  ...
  const fetchMeetings = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.meetings) {
          let loadedMeetings = data.meetings;
  ...
  const handleDownloadCSV = (meeting: any) => {
    const headers = [
      "Meeting ID",
      "Title",
      "Class Level",
    ...
    const row = [
      ...
      meeting.status,
      meeting.type,
      meeting.subjectTitle || "General Lecture",
      meeting.teacherName || "",
      meeting.roomName || "",
      meeting.description || ""
    ];
    ...
    link.setAttribute("download", `meeting_details_${meeting.roomName || meeting.id}.csv`);
        ...
        <div className="text-left">
          ...
          <p className="text-xs text-slate-650 dark:text-slate-400 mt-1 max-w-xl">
            {profile?.role === "teacher" 
              ...
              : "Review scheduled classes, access previous lectures, or join secure live meetings with room codes shared by your educators."}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-left font-sans">
        ...
        <div className="lg:col-span-5">
          {profile?.role === "teacher" ? (
            ...
            /* Student Join Secure Session block */
            <div className="glass-card p-6 border-slate-200 dark:border-white/5 bg-slate-950/40 flex flex-col items-center w-full shadow-2xl space-y-4">
              <div className="w-12 h-12 bg-brand-royal/20 rounded-none flex items-center justify-center border border-white/5 text-blue-500">
                <span className="text-xl">🔒</span>
              </div>
              <div className="text-center">
                <h4 className="text-sm font-extrabold text-white uppercase tracking-wider">Join Secure Session</h4>
              ...
              {errorMsg && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold text-center w-full">
                  {errorMsg}
                </div>
              )}
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-white/10 rounded-none px-4 py-3 text-xs text-white focus:outline-none focus:border-brand-royal transition-colors text-center font-mono tracking-widest uppercase"
              />
              
              <div className="flex gap-3 w-full">
                <button 
                  onClick={() => setView("student-dash")} 
                  className="flex-1 py-2.5 bg-slate-950 hover:bg-slate-900 border border-white/5 text-slate-400 font-bold text-xs transition-colors uppercase tracking-wider"
                >
                  Dashboard
                </button>
                <button 
                  ...
                  className="flex-1 bg-brand-royal hover:bg-brand-royal/90 text-white py-2.5 text-xs font-bold disabled:opacity-50 transition-colors uppercase tracking-wider"
                >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200 dark:border-white/10 rounded-none text-slate-600 dark:text-slate-350 transition-colors flex items-center justify-center"
              ...
              <button
                ...
                className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200 dark:border-white/10 rounded-none text-slate-600 dark:text-slate-350 transition-colors disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center"
                title="Next Month"
              >
                    ...
                    return (
                      <button
                        ...
                        className={`aspect-square relative rounded-none border flex flex-col items-center justify-between p-1.5 transition-all group ${
                          isSelected
                            ? "bg-brand-royal border-brand-royal text-white shadow-md shadow-brand-royal/10 font-bold"
                            : isPast
                              ? "bg-slate-100/50 dark:bg-slate-850/20 border-slate-150 dark:border-white/[0.03] text-slate-400 dark:text-slate-500 hover:border-slate-350 hover:bg-slate-200/50"
                      ...
                      >
                        ...
                        {dayMeetings.length > 0 && (
                          <span className={`text-[8px] font-bold leading-none ${
                            isSelected ? "text-white/95" : isPast ? "text-slate-400 dark:text-slate-500" : "text-brand-royal dark:text-brand-royal-light"
          ...
          {(() => {
            ...
            return (
              <div className="border-t border-slate-200 dark:border-white/5 pt-4 space-y-3">
                <h5 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-display">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {m.status === "Completed" && (
                            <button
                              ...
                              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-extrabold text-[9px] uppercase tracking-wider transition active:scale-95 shrink-0 border border-slate-200 dark:border-white/10 rounded flex items-center gap-1 shadow-sm"
                              title="Download meeting details CSV"
                            >
...
function App() {
  ...
  const shouldShowProtected = auth.isAuthenticated && !!token && !isPublicPage && activeView !== "secure-note-preview";
